...
The computation
is separated into a series rounds.
...
our security analysis does actually make use of this setup.
...
and then defining commitments like this:
$$
\text{Com}(\by, \alpha) := r\cdot H + \sum_i \by_i \cdot G_i
$$
This would have the advantage of a smaller communication cost,
and a slightly improved computation cost, since we have single
blinding factor $\alpha$, rather than an entire vector.

A disadvantage of directly applying this change would be that if our homomorphisms are sparse,
then our proofs wouldn't be able to take advantage of that, since
the commitment now relies on the entire input vector, and so
our proofs would need to use the entire input,
even if the homomorphism for that round only needs a subset. 
...
while also mitigating the overhead
for sparse homomorphisms.
