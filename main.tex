...
\\
On receiving $(\texttt{prove}, \sid, S \subseteq \mathcal{P}, X, x)$ from $P_i$:\\
$\mathcal{F}$ checks that $\sid$ has not been used before.\\
$\mathcal{F}$ checks that $\varphi(x) \stackrel{?}{=} X$.\\
$\mathcal{F}$ sends $(\texttt{proof}, \pid_i, \sid, X)$ to each $P_j \in S$.
