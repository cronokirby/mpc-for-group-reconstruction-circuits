...
\todo{Describe semantics in the isolated case}.
...
\todo{maybe add some more stuff here?}

\subsection{Normalized Form}

\todo{Stuff: Move randomness up, use single input vector, use single homomorphism per layer, depend explicitly on all previous parameters}.

\todo{Then describe the evaluation semantics in this normalized form}.

\section{MPC Protocol for GRCs}

\todo{Talk about the committed model, the various kinds of commitments, the at least one honest model}.
...
\todo{Why leaking intermediate values is allowed.}
